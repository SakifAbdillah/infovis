...
$csv = array_map('str_getcsv', file('area.csv'));
//unset($csv[0]);
//print_r($json_a);
foreach ($csv as $key => $value) {
	$code = substr($value[0], 0, 3);
	foreach ($json_a['areas'] as $key2 => $value2) {
		if($value2["areaCode"] == $code){
			$json_a['areas'][$key2]['totalArea'] = $value[8];
			$json_a['areas'][$key2]['emptySpace'] = $value2['emptySpace'] / $value[8];
		}
	}
}
// $string = file_get_contents("areadata.json");
